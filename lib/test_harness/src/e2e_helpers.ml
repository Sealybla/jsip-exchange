...
(* let connect_as ~port _participant = let where =
   Tcp.Where_to_connect.of_host_and_port [{ host = "localhost"; port }] in
   let%map conn = Rpc.Connection.client where >>| Result.ok_exn in [{ conn }]
   ;; *)

let connect_as ~port (participant : Participant.t) : client Deferred.t =
  let where =
